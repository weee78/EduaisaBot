...
# =============================
# معرف المجموعة الخاصة (يجب التأكد من صحته)
# =============================
OWNER_GROUP_ID = -1003872430815  # تأكد من أن هذا الرقم صحيح لمجموعتك الخاصة
...
# =============================
# قائمة النصائح الصباحية (موسعة إلى 20 نصيحة)
# =============================
MORNING_TIPS = [
    ...
    "📝 **إنتاجية**: استخدم تقنية بومودورو (25 دقيقة عمل / 5 دقائق راحة) لزيادة تركيزك وإنجاز مهامك.",
    "🎓 **تعليم**: منصة Coursera تقدم آلاف الدورات المجانية في مختلف المجالات، اغتنم الفرصة.",
    "💻 **برمجة**: تعلم Git و GitHub لإدارة مشاريعك البرمجية بكفاءة والتعاون مع الآخرين.",
    "🌐 **تقنية**: استخدم متصفح Brave الذي يحجب الإعلانات ويتتبع الخصوصية بشكل افتراضي.",
    "📧 **إنتاجية**: خصص وقتاً محدداً لتصفح البريد الإلكتروني بدلاً من فتحه باستمرار طوال اليوم.",
    "📱 **تطبيقات**: جرب تطبيق Notion لتنظيم ملاحظاتك ومشاريعك بطريقة احترافية.",
    "🖥️ **تقنية**: تعلم اختصارات لوحة المفاتيح للنظام الذي تستخدمه، فهي توفر وقتاً كبيراً.",
    "🎯 **إنتاجية**: حدد أهم 3 مهام يومياً وأنجزها أولاً، هذا يضمن تقدمك في أولوياتك.",
    "📚 **تعليم**: استمع للبودكاست التعليمي أثناء القيادة أو الرياضة، مثل بودكاست 'وعي' أو 'فنجان'.",
    "☁️ **تقنية**: استخدم التخزين السحابي مثل Google Drive أو OneDrive للنسخ الاحتياطي لملفاتك.",
]

# =============================
# قائمة النصائح المسائية (الظهرية/المسائية) موسعة إلى 20 نصيحة
# =============================
AFTERNOON_TIPS = [
    ...
    "📊 **مدير المدرسة**: حلل نتائج الاختبارات وحدد نقاط الضعف لبناء خطط تحسين فعالة.",
    "🎯 **رائد النشاط**: أطلق مبادرة 'طالب الشهر' لتكريم المتميزين أكاديمياً وسلوكياً.",
    "🧑‍🏫 **المعلم**: استخدم استراتيجية 'الصف المقلوب' حيث يدرس الطالب الدرس في المنزل ثم يناقش في الفصل.",
    "🩺 **الموجه الصحي**: نظم محاضرات عن الإسعافات الأولية بالتعاون مع الجهات المختصة.",
    "🤝 **الموجه الطلابي**: تفعيل برنامج الإرشاد الجماعي للطلاب الذين يعانون من صعوبات تعلم.",
    "🏆 **مدير المدرسة**: أطلق جوائز تحفيزية للمعلمين المبدعين لرفع الروح المعنوية.",
    "📝 **المعلم**: استخدم استراتيجية 'التعلم التعاوني' بتقسيم الطلاب إلى مجموعات صغيرة.",
    "📅 **رائد النشاط**: نظم رحلة مدرسية تعليمية لتعزيز الانتماء وتوسيع مدارك الطلاب.",
    "🧘 **الموجه الطلابي**: علم الطلاب تقنيات الاسترخاء والتأمل للتعامل مع ضغوط الاختبارات.",
    "🥗 **الموجه الصحي**: أطلق مبادرة 'مقصف صحي' لتوفير وجبات خفيفة صحية للطلاب.",
]
...
async def daily_afternoon_tips():
    while True:
        ...
        wait_seconds = (next_run - now).total_seconds()
        print(f"💡 النصائح المسائية سترسل بعد {wait_seconds/3600:.2f} ساعة")
        await asyncio.sleep(wait_seconds)
        ...
        try:
            ...
            print(f"❌ فشل إرسال نصيحة مسائية: {e}")
...
# =============================
# الأمر /ask (مع طباعة معرف المجموعة للتأكد)
# =============================
@dp.message(F.text.startswith("/ask"))
async def cmd_ask(message: types.Message):
    print("✅ تم استدعاء /ask")
    print(f"📌 معرف المجموعة الحالية: {message.chat.id}")
    print(f"📌 معرف المجموعة الخاصة المطلوب: {OWNER_GROUP_ID}")
...
async def callbacks(call: types.CallbackQuery):
    ...
    if call.data == "enable_links":
        ...
        if call.data == "enable_ask":
            ...
            # نرسل رسالة تأكيد فقط (النصائح مفعلة بشكل دائم، يمكن إضافة تخزين للحالة لاحقاً)
